...
echo "Calling AWS metadata service to get AWS_PARTITION"
IMDSv1="http://169.254.169.254/latest"
AWS_PARTITION=$(curl "$IMDSv1/meta-data/services/partition")
export AWS_PARTITION

echo "Retrieving IAM roles and credentials for instance"
...
echo "Retrieving EC2_DOCUMENT and default values"
...
echo $AWS_DEFAULT_REGION $AWS_REGION $AWS_ACCOUNT_ID $EC2_INSTANCE_ID

echo "Retrieving autoscaling group name and shutdown lifecycle name (if present)"
...
export AUTO_SCALING_GROUP_NAME SHUTDOWN_LIFECYCLE_NAME
echo $AUTO_SCALING_GROUP_NAME $SHUTDOWN_LIFECYCLE_NAME

echo "Installing python3 and associated libraries"
yum install -y python3 python3-pip python3-wheel python3-devel

echo "wget pip (fixed version 20.2.4) and install"
...
echo "EC2_HOST_IDENTIFIER $EC2_HOST_IDENTIFIER"
...
yum install -y gcc libcurl-devel openssl-devel gcc-c++ postgresql-devel
export PYCURL_SSL_LIBRARY=openssl
pip3 install "apache-airflow[celery,postgres,s3,crypto,google_auth]==1.10.15" "celery[sqs]==4.4.7"
